...
export function RecentActivitiesCard() {
  ...
  const getEntityName = (log: AuditLog): string => {
    if (log.new_value && typeof log.new_value === 'object' && log.new_value !== null) {
      ...
      if ('type' in data && 'person_a_name' in data && 'person_b_name' in data) {
  ...
  const getChangeDetails = (log: AuditLog): string | null => {
    if (log.action === 'UPDATE' && log.old_value && log.new_value) {
      ...
      if (oldData.first_name !== newData.first_name) {
        changes.push(`nama depan dari "${oldData.first_name}" → "${newData.first_name}"`);
      }
      if (oldData.last_name !== newData.last_name) {
        changes.push(`nama belakang dari "${oldData.last_name}" → "${newData.last_name}"`);
      }
      
      if (oldData.gender !== newData.gender) {
        const genderMap: Record<string, string> = { MALE: 'Laki-laki', FEMALE: 'Perempuan' };
        changes.push(`jenis kelamin dari ${genderMap[oldData.gender as string] || oldData.gender} → ${genderMap[newData.gender as string] || newData.gender}`);
      }
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {activities.map((activity) => {
            ...
            const isClickable = normalizedType === 'person' && activity.action !== 'DELETE';
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-slate-900">
                    <span className="font-bold">{activity.user_name || 'Pengguna'}</span>
                    {' '}
                    <span className="text-slate-600">{getActionLabel(activity.action)}</span>
                    {' '}
                    <span className="font-semibold text-slate-900">{getEntityName(activity)}</span>
                  </p>
        ...
        <div className="text-center py-8">
          ...
          <h3 className="mt-2 text-sm font-medium text-slate-900">Belum ada aktivitas</h3>
          <p className="mt-1 text-sm text-slate-500">Aktivitas akan muncul di sini</p>
        </div>
      )}
    </Card>
  );
